...
在本地或容器中完成修改后，GitHub 仓库不会自动同步，需要手动提交并推送。下面提供分步说明，以及可以直接复制执行的命令模板，并解释在 ChatGPT/Codex 网页版里哪里输入命令。

## 在 ChatGPT/Codex 网页版哪里输入命令？
1. 打开对话右上角/侧边栏的 **“终端”/“Files & Terminal”** 面板（通常在模型头图标附近）。
2. 点击 **“New Terminal”**（或“打开终端”）按钮，会出现一个交互式 Shell 窗格，在其中输入本文的 git 命令即可。
3. 如果看不到终端按钮：
   - 你的账号/会话可能未开启代码执行环境；此时可以在本地电脑或你自己的服务器上执行同样的 git 命令。
   - 或者直接让对话里的助手代为执行具体命令（明确告诉要运行的命令），跑完会返回输出结果。

> 终端窗格就是你输入 `git add/commit/push` 等命令的地方，输入后按 Enter 执行即可。
